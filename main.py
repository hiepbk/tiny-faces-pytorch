...
def arguments():
    ...
    parser.add_argument("--traindata",default="./data/widerface/wider_face_split/wider_face_train_bbx_gt.txt" , type=str)
    parser.add_argument("--valdata",default="./data/widerface/wider_face_split/wider_face_val_bbx_gt.txt", type=str)
    parser.add_argument("--dataset-root", default="./data/widerface")
...
def main():
    ...
    train_loader, _ = get_dataloader(
        args.traindata, args, num_templates, img_transforms=img_transforms
    )
    ...
    optimizer = optim.SGD(
        model.learnable_parameters(args.lr),
        lr=args.lr,
    ...
    scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=20, last_epoch=args.start_epoch - 1
    )

    # train and evalute for `epochs`
    accumulated_iter = 0
    tb_log = SummaryWriter(log_dir=os.path.join(args.log_dir, 'tensorboard'))
    for epoch in range(args.start_epoch, args.epochs):
        accumulated_iter = trainer.train(model, loss_fn, optimizer, train_loader, epoch, 
                                         device=device, accumulated_iter= accumulated_iter, tb_log=tb_log)
        scheduler.step()
