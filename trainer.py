...
def train(model, loss_fn, optimizer, dataloader, epoch, device, accumulated_iter, tb_log):
    model = model.to(device)
    model.train()
    
    pbar = tqdm.tqdm(total=len(dataloader), desc="Epoch: [{}]".format(epoch), dynamic_ncols=True)
    ...
    for idx, (img, class_map, regression_map) in enumerate(dataloader):
        ...
        if accumulated_iter % 100==0:
            tb_log.add_scalar('cls_loss', loss_fn.class_average.average, accumulated_iter)
            tb_log.add_scalar('reg_loss', loss_fn.reg_average.average, accumulated_iter)
            tb_log.add_scalar('total_loss', loss.item(), accumulated_iter)

        # print_state(idx, epoch, len(dataloader),
        #             loss_fn.class_average.average,
        #             loss_fn.reg_average.average)
        pbar.set_postfix(total_loss = loss.item(), loss_cls = loss_fn.class_average.average, loss_reg = loss_fn.reg_average.average)
        pbar.update()
        accumulated_iter += 1
